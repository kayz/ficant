...
> **当前状态（2026-09-04）：** 下述一、二章是 2026-07-26 的历史诊断基线，不是当前缺口清单。R1–R8B 的公共实现和本地证据已经完成并合入 `main`；Factor/Position/Subject、RulePack 实际消费、双发行日与 typed price source 等当时缺口已经闭合。当前产品边界见 [`../product/scope.md`](../product/scope.md)，各轮证据见 [`../iterations/`](../iterations/README.md)；AC 点亮仍由独立 private authority/Human 裁决。

---

## 一、历史诊断基线：倒金字塔

2026-07-26 当时各层的实际状态：
...
以下按当时危害排序。每条记录：当时证据 → 违反的条款 → 改进方案 → 归属迭代；其现在时措辞均应按上述日期阅读。
