...
> **当前候选：** R9D 已通过 [PR #69](https://github.com/kayz/ficant/pull/69) 线性合入公共 `main@433d03dd998a1e5829fc7bbc2ec6438e66cbfe00`（tree `1806850afbc1b3ca1690c30cc94a7cc3dd8aa17f`）；Human 于 2026-09-04 选择的 `v0.1.0-alpha.10` 仍未创建。第四次 clean-main 发布预检已通过构建、Server/Worker/UI/Ceph 四镜像扫描、resolved Compose 与空库 migration，随后在 tag 前因 Server 缺少 `FICANT_CODE_COMMIT_SHA` 而 unhealthy 并正确失败。R9E 本地候选已补齐 commit/tree、Server runtime/environment/bootstrap/input、Worker orphan 以及部署状态/回滚身份合同；定向真实 Compose 探针观察到 migration 成功、Server/Worker/UI healthy、`WORKER=200:ok`、`UI=200`。目标证据为 compose security 37 tests（35 passed、2 个显式 live gate skipped、0 failed）、state contract 9/9、repo-policy PASS、实际镜像摘要 resolved validator PASS，修复后独立复审为 Blocker/Major/Minor `0/0/0`；最终 `check-fast.ps1` exit `0`、23/23 步通过，完整 `check.ps1` exit `0`、40/40 步通过。本地候选已完成并可提交/合并；提交/合并与第五次 clean-main preflight 仍待执行，当前边界见 [`2026-09-r9e-release-runtime-identity.md`](../iterations/2026-09-r9e-release-runtime-identity.md)。tag、版本 CI 和测试环境事实仍只能由后续获授权运行提供。
