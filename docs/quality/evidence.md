...
> **当前候选（2026-09-05）：** Human 授权的不可变 tag `v0.1.0-alpha.12` 精确绑定 `42e7c323543401567f8d76166d4d13958dd55f7b`（tree `db9123eabc27d8b1d5c52dd001d57c6f5934dd32`）。同一候选的 clean-main 发布预检 17/17、[版本 CI run 33943037740](https://github.com/kayz/ficant/actions/runs/33943037740) 11/11 与 [release-test run 33943924358](https://github.com/kayz/ficant/actions/runs/33943924358) 10/10 全部成功；Server、Worker、UI 的 SHA/版本标签分别绑定同一不可变 OCI index digest，三者均有 SPDX SBOM 与 SLSA provenance，连同锁定 Ceph RGW 的 HIGH/CRITICAL 扫描均为 0 result。GitHub Deployment `6277018679` 在 `test` Environment 完成 27/27 migration、PostgreSQL/Ceph RGW/Server/Worker/UI 健康与 smoke，未触发回滚。该结果证明精确 `alpha.12` 的版本与测试环境交付，不等于完整业务 UAT 或生产发布；`v0.1.0-alpha.11@791555b22b6ef8c847622621c860a8789ba9e32d` 仍原位保留为 Rust/Web 失败且未部署的历史候选。当前边界见 [`2026-09-r9g-linux-release-parity.md`](../iterations/2026-09-r9g-linux-release-parity.md) 与 [发布说明](../delivery/release-notes.md)。
