...
- **合并前独立审查修正：** Draft 公共候选审查发现四项与冻结语义不一致但可在既有写路径内 forward-only 修复的问题：native Bond engine 的价格变化按 registered face 计量，而 application 曾固定除以 100，导致非 100 面值的相同名义持仓错误缩放；Position content hash 未承诺 CurveNode / curve binding 与非零 Instrument binding 的内容哈希，portfolio content hash 也只写入 lineage object id；domain constructor 未拒绝非 canonical FactorId 顺序；descriptor 判据未逐字段锁住新增 Exposure 消息与 Bond convention enum。Human 的既有授权覆盖这种不改变公共 wire shape、算法 convention 或 acceptance 的机械修复；修正使用 registered face 精确归一化，把已验证 topology evidence 排序去重后纳入 position hash、把完整 LineageRef 纳入 portfolio hash，并加严 domain 与 descriptor 判据。未修改 expected / Oracle / Golden / matrix / canonical / allowlist 或任何冻结公共字段。
...
- Draft 候选的独立合并前审查先只新增不弱化的判据：application 非 100 registered face 对照 exit 1（3/4，100 面值为 `15000000000`、200 面值被错误放大为 `30000000000`）；domain canonical FactorId 顺序判据 exit 1（2/3），修正排序后继续暴露 portfolio lineage 只改 hash 但 content hash 相等并再次 exit 1（2/3）。随后按 §5 修正；合并前 strict Clippy 又以新增 domain 测试函数 115/100 行失败，机械拆成五个不弱化断言的小测试后，domain 5/5、application 4/4、descriptor 17/17 均 exit 0。原 application fixture 的 native-engine stub 同步按 registered face 线性缩放，新增对照证明相同 quantity 的风险值不随债券面额定义改变，而 definition lineage 仍使 content hash 改变；这些 RED 均不是 checkpoint，也未改写原实现期 RED 事实。

**最终同一候选的针对性证据（全部 exit 0）：**

- `cargo test --offline --locked -p ficant-domain --test r4d_a_bond_krd_contracts`：5/5。
- `cargo test --offline --locked -p ficant-application --test r4d_a_bond_krd_contracts`：4/4；两只 Bond × 三个 Factor 的完整向量与 exact totals 成立，bump / direction 改变结果，registered face 归一化成立，且 CurveNode / curve binding 与非零 Instrument binding hash 均进入稳定排序的 position input evidence；加入纳入敞口的非 Bond 时 curve 与 bond engine call count 均为 0。Root 终态静态复核另发现 UnitRef 已校验但 registry scale / precision 尚未承重，先只加入低精度 DV01、低 precision notional 与错误 rate dimension 三类负向 fixture，同一命令 exit 1（2/3，低精度 DV01 被错误接受）；随后在 application 边界补齐 exact owner / dimension / scale / precision 校验并把 rate Unit definition hash 纳入逐仓位 lineage，重跑 3/3。该实现期 RED 与合并前审查 RED 都不是 checkpoint，未修改既有 expected。
...
- 在同一合并前修正候选上导入六个 Windows User 级 `FICANT_TEST_*` 变量（未输出密钥）执行 `pwsh -NoProfile -NonInteractive -File scripts/check.ps1 -IncludeIntegration`：exit 0。除上述完整检查外，真实 PostgreSQL / Ceph 范围包括 migration 4/4、Phase 4C lease 1/1、execution closure 3/3、production Worker 1/1、Phase 1 business loop 1/1、negative invariants 13/13、Phase 2B / 2C / 2D 各 1/1、Phase 3A registry / parity 各 1/1、Phase 3B publication 1/1。
...
**终态审计：** `git diff --check` exit 0。相对固定 execution base `4e472a8993b5d2a5c4a5c69bf078c9659d19e2de` 的最终 PR diff 共 51 个变更路径，逐项属于冻结的 63 项允许清单，范围外写入为 0。受保护的 allowlist、canonical schema、Golden / Oracle、Phase 2C / 2D matrix、C++ / C ABI、既有 rates / position / factor proto 与 `rates.rs` 对 base 的 diff 均为空；13 项冻结 Git object ID 逐项一致，allowlist 内容仍为 `[]`。变更的 proto 只有获准的 `market/v1/definition.proto`、`market/v1/fact.proto` 与新建 `research/v1/exposure.proto`，点名禁止的 position / factor / health / constraint / policy proto 为 0。最终公共候选身份由 PR #51 的 remote head 与本地 branch head 精确一致承载；brief 不自引用包含自身的 commit SHA。
