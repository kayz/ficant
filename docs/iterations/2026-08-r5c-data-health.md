...
把 ADR-0017 §三的数据健康度自检落成 ADR-0016 形态的一等 `AnalyticsService`：请求精确绑定一个不可变 `PositionSnapshot`、一个带 exact VersionRef 与 content hash 的完整阈值 profile，并可选绑定一个 verified `DataSnapshot`；响应列出机器可判的具体 issue、实际检查范围、request fingerprint、coverage 与 lineage。服务只读取现有仓库与不可变 blob，不写状态、不产生 journal、lease 或隐式默认配置。
...
| AC36 · 陈旧 | `evaluated_at - observed_at` 严格大于 profile 的秒级上限才触发 `STALE_POSITION_SNAPSHOT`；可选 DataSnapshot 对 `as_of` 使用独立上限。等于上限不触发，早于 observed/as_of 的评估时间失败关闭。MarketTime 使用完整 instant，不只比较日期。 |
...
| R5b 门禁 · 闭集 | descriptor inventory 新增且只新增 `DataHealthService.GetDataHealthReport → DataHealthReport` success arm，并归为 Composition；总数精确 67 / 4 / 63。删除 `SINGLE_POSITION` 后三个非组合理由均至少有一个真实 arm。既有六个负向 fixture 全保留，新增“删除 health coverage”第七个 fixture；七项均真实 exit 1，已分类 base inventory exit 0。 |
| 生产组合 | 真实 server builder、gRPC-Web mux 与生成客户端可命中 `DataHealthService/GetDataHealthReport`；服务使用现有 Postgres PositionSnapshot/DataSource、verified S3/blob read 与 canonical decoder，不以 test-only repository 冒充生产路由。scope 固定为 `data-health:read`。 |
| I3 / I8 · 绑定与确定性 | profile 的 exact VersionRef 与 content hash 先验核验并进入 request fingerprint、report content hash 与 LineageRef；同一 ref/version 不同内容失败关闭。issue 按 `(code, position_id, data_source id/version)` 排序；所有计数、比例、输入引用、position-set state、coverage 与 issue 进入 report content hash。相同请求及存储事实产生逐位相同 protobuf 输出；换 profile 必须同时改变 fingerprint、hash 与 lineage。 |
...
11. coverage gate 的 expected / inventory 变化必须先由新增公共 RPC 使旧闭集真实 RED，再按本 brief 的 Human 授权精确新增一个 Composition arm；不得删除旧 arm、改已有分类或放宽未知默认失败。`SINGLE_POSITION` 删除与 67 / 4 / 63 数字、七个负向 fixture须在同一可审阅 diff 呈现。
...
- 管理员级阈值 registry、数据库配置、环境变量默认、按主体/租户的隐式覆盖或 partial override。若 Human 要平台统一阈值，应先拆出独立配置注册轮次，不得把常量藏进 server。
...
- 新 migration、policy / constraint / factor / position proto、C++ / C ABI / native、domain-packs、税制、AC09、AC37、authority 三件套、SPEC、ACCEPTANCE、MANUAL、任何 ADR、版本 tag、CI/CD 或发布。
...
| 6 | `content_hash` | `ficant.core.v1.Sha256`；覆盖字段 1–5 的规范编码，必须先验精确。 |
...
- 新增 `GetDataHealthReportRequest`：`subject_ref = 1`、`position_snapshot_id = 2`、可选 `data_snapshot_id = 3`、`evaluated_at = 4`、`threshold_profile = 5`。请求不接受现成 issue、health score、计算结果或逐字段 override。
...
- domain 新增不可变 threshold、issue、snapshot evaluation、verified-empty capability 与 report 值对象；profile hash、fingerprint、时间、比例、排序、issue / coverage 同源、position-set state 与 report hash 在 domain 校验。`PositionSnapshot` 允许 positions 为空但继续要求非空 lineage；Coverage 新增只消费 verified-empty capability 的 health empty constructor，既有 complete constructor 与三个 carrier 不变量保持。profile 的 exact ref + hash 加入报告 lineage。
- application 新增只读 DataHealth use case，复用 `PositionSnapshotRepository`、`VerifiedSnapshotReader`、`CanonicalSnapshotDecoder` 与 `DataSourceRepository`。API 只做解析 / 映射，production server 组合现有 repository、blob reader、integrity sink 与 decoder；不新增 persistence port 或 migration。
...
- R5b coverage inventory 只做与新服务严格对应的前进：新增健康 success arm 为 Composition，删除零使用 `SinglePosition` reason，三个既有 composition arm与 63 个既有非组合 arm的分类逐项不变。coverage descriptor 总数从 66 / 3 / 63 变为 67 / 4 / 63。
...
- **已裁决——阈值来源与覆盖性：** Human 批准 R5c 使用请求内完整 `DataHealthThresholdProfile`，包含 exact VersionRef 与 content hash；无产品默认、环境默认、数据库 registry 或逐字段 override。换阈值必须换 exact version / hash，完整 profile 进入 OperationFingerprint，exact ref + hash 进入 LineageRef，报告只声明“按该 profile”。这保持 ADR-0016 的无状态查询并避免代码常量。若以后要管理员统一阈值或平台默认，必须另开配置注册轮次，不能实施后补。
...
- **已裁决——第 4 个 composition carrier：** Human 批准按 R5b 既有定义把 DataHealthReport 归为 Composition 并携带 coverage；仅 health carrier 新增窄 `0 / 0` 空 coverage，现有三个 carrier 的正分母不变量保持。相应 inventory 前进到 67 / 4 / 63，新增第七个删 health coverage 负向 fixture。
...
- **已裁决——自管门禁精确变化：** Human 授权在新增 RPC 令旧闭集真实 RED 后，把 descriptor inventory 精确增加一个 Composition arm、删除零使用 reason、更新 67 / 4 / 63 文本并新增删 health coverage fixture。既有 66 arm、六个负向 fixture、未知默认失败与其余分类必须保持；这不是因业务测试失败而 rebaseline。
...
- transport / gate RED：先新增未带 coverage、未进 inventory 的 health success arm，固定 Buf descriptor 与 coverage gate 必须真实非零；不得先改 expected。生成输出、API mapping、生产 mux、67 / 4 / 63 闭集和七个负向 fixture全部转绿后才形成 transport / production checkpoint。
...
- `cargo test --offline --locked -p ficant-storage --test migration_acceptance -- --test-threads=1`（仍须完整 4/4，且 migration tree OID 不变）
...
- contract checkpoint：profile exact ref / hash、整数阈值、完整 MarketTime、稳定 issue、一次 evaluation、verified-empty capability、coverage / fingerprint / report hash / lineage 全部由 domain 直接测试覆盖；R5c `5 / 5` 与 R5b coverage 回归 `2 / 2` 转绿后成立。
- application checkpoint：exact PositionSnapshot、可选 verified DataSnapshot、exact DataSource 与完整 profile 读取转绿；应用层 `11 / 11` 证明最差可计算 Bond+Futures fixture 的 KRD 完整 success、健康/最差 pair 的 position / totals 数值与算法口径不变，以及 AC17 CapitalUse 继续失败关闭；既有 position snapshot `2 / 2` 回归通过。API 层 `10 / 10` 中的独立 wire 判据再以同一组已编码计算请求 bytes 在 WARNING 查询前后调用真实 adapter，比较完整 `CalculateKeyRateDv01Response.encode_to_vec()`，重复健康请求还比较完整 `GetDataHealthReportResponse.encode_to_vec()`，并证明 curve / bond / RulePack parser / delivery 四类 engine call count 均未因健康查询变化；两层同时转绿后 checkpoint 才成立。
- transport / production checkpoint：生成输出、API、gRPC-Web 与真实 server builder 完成后，API health `10 / 10`、server SIT `1 / 1`、descriptor `20 / 20`、67 / 4 / 63 闭集、七个负向 fixture与 51 条分层 fixture全部转绿后成立。
...
| `cargo test --offline --locked -p ficant-application --test r5c_data_health_contracts` | exit 0；`11 / 11` |
...
| `pwsh -NoProfile -NonInteractive -File scripts/check-coverage.ps1` | exit 0；目标 descriptor 判据 `1 / 1`（其余 19 filtered）；67 个 success arm 全部显式分类，4 个 composition carrier、63 个使用 3 个实际理由的非组合 arm |
...
**生成确定性：** 按 `interface/README.md` 使用固定 Buf 1.56.0 在两个独立临时输出树执行完整生成；两棵树文件集合均为 `74`，规范化 SHA-256 比较 `74 / 74` 相同、mismatch `0`。§6 点名的五个候选生成文件（Rust flat package两份、Python两份、TypeScript一份）均与两次生成结果逐文件相同。`interface/buf.gen.yaml` base-to-candidate diff 只有把 `ficant.research.v1.DataHealthService` 加入既有 Python gRPC `types` 闭集；插件、version、revision、out、option 与其他 service 条目均未改。
...
- 使用相同固定工具链执行 `pwsh -NoProfile -NonInteractive -File scripts/check.ps1`，exit 0。分层 fixture `51` assertions、coverage 七个负向 fixture、严格 Clippy、Rust 全量测试（含 DataHealth API wire `10 / 10`）、descriptor `20 / 20`、C++ `8 / 8`、主 matrix `36 / 36`、Phase 2B `16 / 16`、Phase 2C `18 / 18` 与 Oracle `3 / 3`、Phase 2D `18 / 18` 与 Oracle `3 / 3`、Python、Phase 2E live、Phase 3A、Web typecheck / build 与 Web `35 / 35` 全部通过。
...
- `-IncludeIntegration` 第一次在环境测试阶段为 exit 1：migration `0 / 4` 均在测试逻辑前以 `PoolTimedOut` 报告 `FICANT_TEST_DATABASE_URL` 不可达；只读核验确认 Docker Desktop Linux engine 未运行。启动既有本地 engine 后，PostgreSQL 与 Ceph RGW 恢复 healthy，单独 migration acceptance 重取为 exit 0、`4 / 4`，随后完整 `-IncludeIntegration` 从头重跑并 exit 0。该外部前置失败未记为候选通过，变量值全程未输出。

**范围、受保护事实与停机条件复核：** `HEAD == refs/remotes/origin/main == 2f84601f0b0558d60b79f630e8cdf03e3ff92311`，候选仍是该冻结 base 上的未提交 forward-only 工作树。§6 冻结清单保持 `30` 项原文不变；实际 tracked + untracked changed-path 集合精确 `30 / 30`，extra `0`、missing `0`，`git diff --check` exit 0。§6 列出的 `24 / 24` 个冻结 blob / tree OID 逐项与 base 一致，均无 tracked diff 或 untracked child；`scripts/layering-allowlist.json` 内容仍精确为 `[]`。Acceptance sentence、AC17 与 AC35 回归均由上述机械判据满足；公共候选未改 authority。R5c 因此已形成完整本地自测候选，但在公共候选独立审查、rebase merge、authority 精确绑定，以及 Human 单独签署 wire 可见限定并逐条批准前，不宣称 AC36 已正式点亮。

## 7. 残余风险

- 请求绑定阈值 profile 的 exact ref / hash 同时进入 fingerprint 与 lineage，能保证可复现与不藏代码常量，但不等于平台管理员认可的统一健康政策。若实际产品需要租户级强制 profile，必须新增独立、可审计的配置注册与权限迭代；R5c 报告只能按 exact profile 解读。
...
- R5c 新 RPC 会按设计触发 R5b coverage 闭集门禁。这是预期 RED，不得被当成意外失败后静默加入 inventory；§5 的精确 Human 授权与 67 / 4 / 63、七个负向 fixture是唯一允许的前进路径。
