...
> **当前交付状态（2026-09-04）：** R9D 已通过 [PR #69](https://github.com/kayz/ficant/pull/69) 线性合入公共 `main@433d03dd998a1e5829fc7bbc2ec6438e66cbfe00`（tree `1806850afbc1b3ca1690c30cc94a7cc3dd8aa17f`）；Human 已选择 `v0.1.0-alpha.10`。第四次 clean-main 发布预检已通过构建、Server/Worker/UI/Ceph 四镜像扫描、resolved Compose 与空库 migration，随后在 tag 前因 Server 缺少 `FICANT_CODE_COMMIT_SHA` 而 unhealthy 并正确失败。R9E 已在本地候选补齐发布运行时身份合同；定向真实 Compose 探针、compose security 37 tests（35 passed、2 个显式 live gate skipped）、state contract 9/9、repo-policy、实际镜像摘要 resolved validator 与修复后独立复审 Blocker/Major/Minor `0/0/0` 均已通过；最终 `check-fast.ps1` exit `0`、23/23 步通过，完整 `check.ps1` exit `0`、40/40 步通过。本地候选已完成并可提交/合并；提交/合并与第五次 clean-main preflight 仍待执行。目标 tag 仍不存在，最近既有 tag 仍为 `v0.1.0-alpha.9`；本段不宣称 `alpha.10` 镜像、版本 CI 或测试环境已经交付。
...
- R9E 让发布 Compose、本地 preflight、GitHub 交付入口、测试机部署与回滚共享已授权 commit/tree：Server/Worker 接收同一 Code 身份，拉取或构建后派生 Server/Worker 实际镜像 config digest 与 Worker native source digest；Server 补齐固定测试环境摘要、bootstrap bearer/tenant/actor/owner/role 与文件/PostgreSQL 输入绑定，Worker 补齐受控 orphan grace/interval。`deploy.sh` 收紧为 `<commit> <tree>`，`current.env` / `previous.env` 保存 tree、Server runtime、Worker runtime/source，旧状态只允许显式 legacy 零值兼容。定向本地真实 Compose 探针已观察到 migration 成功、Server/Worker/UI healthy、`WORKER=200:ok`、`UI=200`；实际镜像摘要的 resolved validator 为 PASS，修复后独立复审为 Blocker/Major/Minor `0/0/0`；最终 `check-fast.ps1` 23/23 和完整 `check.ps1` 40/40 均以 exit `0` 通过。本地候选已完成并可提交/合并；提交/合并与第五次 clean-main preflight 仍待执行。当前边界见 [R9E brief](../iterations/2026-09-r9e-release-runtime-identity.md)。
...
- 本节只是 pre-tag 候选说明。门禁收口与首次完整本地证据见 [R9A brief](../iterations/2026-09-r9a-release-gate-closure.md)，preflight 构建身份修复见 [R9B brief](../iterations/2026-09-r9b-release-identity-binding.md)，UI 运行时修复见 [R9C brief](../iterations/2026-09-r9c-ui-runtime-cve.md)，Compose 候选绑定见 [R9D brief](../iterations/2026-09-r9d-compose-candidate-binding.md)，当前运行时身份闭环状态见 [R9E brief](../iterations/2026-09-r9e-release-runtime-identity.md)；版本 CI、GHCR、SBOM/provenance、测试环境部署与回滚只能由后续不可变 tag 提供外部证据。
