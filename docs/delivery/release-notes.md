...
- `alpha.10` 的远端 Linux CI 暴露两个独立交付缺口：Rust、Web、business-loop 与 reproducibility 的 Server/Worker 编译没有从授权 job 接收 commit/tree，contract breaking baseline 则不是公共 `main` 可达对象。R9F 保持两个 `build.rs` 的失败关闭语义，改由授权 job 输出精确身份并由所有消费者显式传递；reproducibility 在真实 worktree 绑定身份后让两份无 `.git` archive 继承；contract baseline 改绑到接口树等价的公共祖先。最终目标 gate、真实 contract/reproducibility gate、23/23 快速检查、40/40 标准检查与独立复审已经通过；该本地候选待合入，未产生新的远端 CI 通过证据。当前边界见 [R9F brief](../iterations/2026-09-r9f-ci-source-identity.md)。
