...
- **R9F 合入状态（2026-09-05）：** R9E 已通过 [PR #70](https://github.com/kayz/ficant/pull/70) 合入 `main@6b194996cce06d8fefee91b130e28869a3ae5293`（tree `2f5f73381c0701e061802a56f34c7aa4f7e8a3ff`），第五次 clean-main preflight 17/17 步通过。不可变 tag `v0.1.0-alpha.10` 已推送；[版本 CI](https://github.com/kayz/ficant/actions/runs/33889960292) 因编译身份传播和 contract baseline 两个缺口失败，[release-test](https://github.com/kayz/ficant/actions/runs/33890473662) 的 7/7 jobs skipped，未构建或推送版本应用镜像、未登录测试机、未执行 migration 或部署。R9F 让 authorize job 输出经 tag/current-main 双重验证的 commit/tree，并显式注入 rust、Web Worker/Server、business-loop 与无 `.git` archive 的 reproducibility；contract baseline 同时改绑到公共可达且接口树等价的祖先。最终目标 gate、真实 contract/reproducibility gate、23/23 快速检查、40/40 标准检查与独立复审均已通过，并由 [PR #71](https://github.com/kayz/ficant/pull/71) 合入；仍没有新的远端 CI 通过证据。`alpha.10` 保持原位且不重跑；后续部署必须等待 Human 明确选择新的 forward-only 版本，详见 [R9F brief](../iterations/2026-09-r9f-ci-source-identity.md)。
