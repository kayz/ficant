...
$steps = @(
    ...
    New-FicantCheckStep -Name 'R8A independent Portfolio360 Decimal Oracle' -FilePath 'uv' -ArgumentList @('run', '--offline', '--locked', '--project', 'python', 'python', '-m', 'pytest', 'tests/oracle/portfolio360/test_r8a_portfolio_metric_decimal_oracle.py', '-q')
    ...
    New-FicantCheckStep -Name 'R8A deterministic local contract package' -FilePath 'pwsh' -ArgumentList @('-NoProfile', '-NonInteractive', '-File', 'scripts/test-contract-package.ps1')
...
if ($IncludeIntegration) {
    $steps += @(
        ...
        New-FicantCheckStep -Name 'R8A Portfolio360 PostgreSQL integration' -FilePath 'cargo' -ArgumentList @('test', '--offline', '--locked', '-p', 'ficant-storage', '--test', 'r8a_portfolio_postgres', '--', '--test-threads=1')
        New-FicantCheckStep -Name 'R8A Portfolio360 production gRPC/gRPC-Web integration' -FilePath 'cargo' -ArgumentList @('test', '--offline', '--locked', '-p', 'ficant-server', '--test', 'r8a_portfolio360_sit', '--', '--ignored', '--test-threads=1')
