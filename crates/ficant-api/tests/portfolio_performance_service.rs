...
use ficant_api::{
    PlatformApplication, PlatformPort, PortfolioPerformanceBackend,
...
#[async_trait]
impl PortfolioPerformanceBackend for RejectingBackend {
    async fn get_performance(
        &self,
        principal: &AuthorizedPrincipal,
        context: RequestedNormalizedPortfolioContext,
    ) -> ApplicationResult<PortfolioPerformanceBackendResult> {
...
async fn authorized_exact_context_reaches_typed_backend_once() {
    fn assert_service<T: PortfolioPerformanceService>() {}
    assert_service::<PortfolioPerformanceGrpcService>();
    ...
    .get_portfolio_performance(Request::new(valid_request()))
    .await
    .unwrap()
    .into_inner();
    let Some(pb::get_portfolio_performance_response::Result::Error(error)) = response.result else {
...
async fn role_and_scope_drift_fail_before_backend() {
    for (role, scopes) in [
        ...
        let response = service(backend.clone(), role, scopes)
            ...
            .unwrap()
            .into_inner();
        let Some(pb::get_portfolio_performance_response::Result::Error(error)) = response.result
        else {
...
#[tokio::test]
async fn incomplete_or_non_exact_context_fails_before_backend() {
    let backend = Arc::new(RejectingBackend {
        calls: AtomicUsize::new(0),
    });
    let service = service(
    ...
    let missing = service
        .get_portfolio_performance(Request::new(pb::GetPortfolioPerformanceRequest {
            context: None,
        }))
        .await
        .unwrap()
    ...
    assert!(matches!(
        missing.result,
        Some(pb::get_portfolio_performance_response::Result::Error(_))
    ));
    ...
    drift
        ...
        .content_hash = None;
    let drift = service
        .get_portfolio_performance(Request::new(drift))
        .await
        .unwrap()
        .into_inner();
    assert!(matches!(
...
fn service(
    ...
    role: PlatformRole,
    scopes: Vec<&str>,
) -> PortfolioPerformanceGrpcService {
    ...
    PortfolioPerformanceGrpcService::new(platform, backend, KEY).unwrap()
}

fn valid_request() -> pb::GetPortfolioPerformanceRequest {
    let exact = exact_lineage("01ARZ3NDEKTSV4RRFFQ69G5F10", b"portfolio");
    pb::GetPortfolioPerformanceRequest {
